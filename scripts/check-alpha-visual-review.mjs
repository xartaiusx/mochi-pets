...
assert(browserPresence.data?.hudAction?.state?.rosterCabinetProof === true, 'HUD action proof must include the roster cabinet proof');
assert(browserPresence.data?.hudAction?.state?.rosterCabinetId === 'jade-roster-cabinet', 'HUD action proof must include the Jade Roster Cabinet id');
assert(browserPresence.data?.hudAction?.state?.rosterCabinetName === 'Jade Roster Cabinet', 'HUD action proof must include the Jade Roster Cabinet name');
assert(browserPresence.data?.hudAction?.state?.rosterCabinetScore >= 30, 'HUD action proof must include a passing roster cabinet score');
assert(browserPresence.data?.hudAction?.state?.rosterCabinetRequiredScore === 30, 'HUD action proof must include the roster cabinet requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.rosterCabinetSpiritIds) && browserPresence.data.hudAction.state.rosterCabinetSpiritIds.length === 3, 'HUD action proof must include the full roster cabinet spirit roster');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.rosterCabinetPartyIds) && browserPresence.data.hudAction.state.rosterCabinetPartyIds.length === 3, 'HUD action proof must include the full roster cabinet party');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.rosterCabinetSlotLabels) && browserPresence.data.hudAction.state.rosterCabinetSlotLabels.length === 3, 'HUD action proof must include three roster cabinet storage labels');
assert(browserPresence.data?.hudAction?.state?.rosterCabinetTagClaimed === true, 'HUD action proof must include the no-real-value roster cabinet tag proof');
assert(browserPresence.data?.hudAction?.state?.blossomCradleProof === true, 'HUD action proof must include the blossom cradle proof');
assert(browserPresence.data?.hudAction?.state?.blossomCradleId === 'jade-blossom-cradle', 'HUD action proof must include the Jade Blossom Cradle id');
assert(browserPresence.data?.hudAction?.state?.blossomCradleName === 'Jade Blossom Cradle', 'HUD action proof must include the Jade Blossom Cradle name');
assert(browserPresence.data?.hudAction?.state?.blossomCradleScore >= 48, 'HUD action proof must include a passing blossom cradle score');
assert(browserPresence.data?.hudAction?.state?.blossomCradleRequiredScore === 48, 'HUD action proof must include the blossom cradle requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.blossomCradleSpiritIds) && browserPresence.data.hudAction.state.blossomCradleSpiritIds.length === 3, 'HUD action proof must include the full blossom cradle spirit roster');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.blossomCradlePartyIds) && browserPresence.data.hudAction.state.blossomCradlePartyIds.length === 3, 'HUD action proof must include the full blossom cradle party');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.blossomCradleCareIds) && browserPresence.data.hudAction.state.blossomCradleCareIds.length === 3, 'HUD action proof must include all blossom cradle care ids');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.blossomCradleMilestoneLabels) && browserPresence.data.hudAction.state.blossomCradleMilestoneLabels.length === 3, 'HUD action proof must include three blossom cradle milestones');
assert(browserPresence.data?.hudAction?.state?.blossomCradleTotalBond >= 15, 'HUD action proof must include enough blossom cradle bond');
assert(browserPresence.data?.hudAction?.state?.blossomCradleRibbonClaimed === true, 'HUD action proof must include the no-real-value blossom cradle ribbon proof');
...
assert(browserPresence.data?.hudAction?.state?.provisionCatalogProof === true, 'HUD action proof must include the provision catalog proof');
assert(browserPresence.data?.hudAction?.state?.provisionCatalogId === 'jade-provision-catalog', 'HUD action proof must include the Jade Provision Catalog id');
assert(browserPresence.data?.hudAction?.state?.provisionCatalogName === 'Jade Provision Catalog', 'HUD action proof must include the Jade Provision Catalog name');
assert(browserPresence.data?.hudAction?.state?.provisionCatalogScore >= 50, 'HUD action proof must include a passing provision catalog score');
assert(browserPresence.data?.hudAction?.state?.provisionCatalogRequiredScore === 50, 'HUD action proof must include the provision catalog requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.provisionCatalogItemIds) && browserPresence.data.hudAction.state.provisionCatalogItemIds.includes('jade-thread-charm'), 'HUD action proof must include the Jade Thread Charm provision catalog item');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.provisionCatalogItemIds) && browserPresence.data.hudAction.state.provisionCatalogItemIds.includes('lantern-harmony-tea'), 'HUD action proof must include Lantern Harmony Tea provision catalog item');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.provisionCatalogItemIds) && browserPresence.data.hudAction.state.provisionCatalogItemIds.includes('jade-mooncake-box'), 'HUD action proof must include Jade Mooncake Box provision catalog item');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.provisionCatalogCareItemIds) && browserPresence.data.hudAction.state.provisionCatalogCareItemIds.includes('jade-mooncake-box'), 'HUD action proof must include the catalog care item proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.provisionCatalogRouteItemIds) && browserPresence.data.hudAction.state.provisionCatalogRouteItemIds.includes('lantern-harmony-tea'), 'HUD action proof must include the catalog route item proof');
assert(browserPresence.data?.hudAction?.state?.provisionCatalogPresenceCount >= 2, 'HUD action proof must include two-tester provision catalog presence');
assert(browserPresence.data?.hudAction?.state?.provisionCatalogSealClaimed === true, 'HUD action proof must include the no-real-value provision catalog seal proof');
assert(browserPresence.data?.hudAction?.state?.battleKitProof === true, 'HUD action proof must include the battle item kit proof');
assert(browserPresence.data?.hudAction?.state?.battleKitId === 'jade-battle-kit', 'HUD action proof must include the Jade Battle Kit id');
assert(browserPresence.data?.hudAction?.state?.battleKitName === 'Jade Battle Kit', 'HUD action proof must include the Jade Battle Kit name');
assert(browserPresence.data?.hudAction?.state?.battleKitScore >= 48, 'HUD action proof must include a passing battle kit score');
assert(browserPresence.data?.hudAction?.state?.battleKitRequiredScore === 48, 'HUD action proof must include the battle kit requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleKitItemIds) && browserPresence.data.hudAction.state.battleKitItemIds.includes('lantern-harmony-tea'), 'HUD action proof must include Lantern Harmony Tea battle kit proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleKitItemIds) && browserPresence.data.hudAction.state.battleKitItemIds.includes('jade-thread-charm'), 'HUD action proof must include Jade Thread Charm battle kit proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleKitItemIds) && browserPresence.data.hudAction.state.battleKitItemIds.includes('jade-mooncake-box'), 'HUD action proof must include Jade Mooncake Box battle kit proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleKitPartyIds) && browserPresence.data.hudAction.state.battleKitPartyIds.length === 3, 'HUD action proof must include the full battle kit party');
assert(browserPresence.data?.hudAction?.state?.battleKitPresenceCount >= 2, 'HUD action proof must include two-tester battle kit presence');
assert(browserPresence.data?.hudAction?.state?.battleKitTagClaimed === true, 'HUD action proof must include the no-real-value battle kit tag proof');
assert(browserPresence.data?.hudAction?.state?.remedyPouchProof === true, 'HUD action proof must include the remedy pouch proof');
assert(browserPresence.data?.hudAction?.state?.remedyPouchId === 'jade-remedy-pouch', 'HUD action proof must include the Jade Remedy Pouch id');
assert(browserPresence.data?.hudAction?.state?.remedyPouchName === 'Jade Remedy Pouch', 'HUD action proof must include the Jade Remedy Pouch name');
assert(browserPresence.data?.hudAction?.state?.remedyPouchScore >= 50, 'HUD action proof must include a passing remedy pouch score');
assert(browserPresence.data?.hudAction?.state?.remedyPouchRequiredScore === 50, 'HUD action proof must include the remedy pouch requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchItemIds) && browserPresence.data.hudAction.state.remedyPouchItemIds.includes('lantern-harmony-tea'), 'HUD action proof must include Lantern Harmony Tea remedy pouch proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchItemIds) && browserPresence.data.hudAction.state.remedyPouchItemIds.includes('jade-thread-charm'), 'HUD action proof must include Jade Thread Charm remedy pouch proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchItemIds) && browserPresence.data.hudAction.state.remedyPouchItemIds.includes('jade-mooncake-box'), 'HUD action proof must include Jade Mooncake Box remedy pouch proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchConditionIds) && browserPresence.data.hudAction.state.remedyPouchConditionIds.includes('lantern-ward'), 'HUD action proof must include Lantern Ward remedy pouch condition proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchConditionIds) && browserPresence.data.hudAction.state.remedyPouchConditionIds.includes('goldleaf-tempo'), 'HUD action proof must include Goldleaf Tempo remedy pouch condition proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchConditionIds) && browserPresence.data.hudAction.state.remedyPouchConditionIds.includes('skybell-guard'), 'HUD action proof must include Skybell Guard remedy pouch condition proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.remedyPouchPartyIds) && browserPresence.data.hudAction.state.remedyPouchPartyIds.length === 3, 'HUD action proof must include the full remedy pouch party');
assert(browserPresence.data?.hudAction?.state?.remedyPouchPresenceCount >= 2, 'HUD action proof must include two-tester remedy pouch presence');
assert(browserPresence.data?.hudAction?.state?.remedyPouchTagClaimed === true, 'HUD action proof must include the no-real-value remedy pouch tag proof');
...
assert(browserPresence.data?.hudAction?.state?.bondGiftProof === true, 'HUD action proof must include the bond gift rite proof');
assert(browserPresence.data?.hudAction?.state?.bondGiftRiteId === 'jade-bond-gift-rite', 'HUD action proof must include the Jade Bond Gift Rite id');
assert(browserPresence.data?.hudAction?.state?.bondGiftRiteName === 'Jade Bond Gift Rite', 'HUD action proof must include the Jade Bond Gift Rite name');
assert(browserPresence.data?.hudAction?.state?.bondGiftScore >= 36, 'HUD action proof must include a passing bond gift score');
assert(browserPresence.data?.hudAction?.state?.bondGiftRequiredScore === 36, 'HUD action proof must include the bond gift requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.bondGiftItemIds) && browserPresence.data.hudAction.state.bondGiftItemIds.includes('jade-mooncake-box'), 'HUD action proof must include the Jade Mooncake Box bond gift');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.bondGiftItemIds) && browserPresence.data.hudAction.state.bondGiftItemIds.includes('lantern-harmony-tea'), 'HUD action proof must include the Lantern Harmony Tea bond gift');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.bondGiftItemIds) && browserPresence.data.hudAction.state.bondGiftItemIds.includes('jade-thread-charm'), 'HUD action proof must include the Jade Thread Charm bond gift');
assert(browserPresence.data?.hudAction?.state?.bondGiftPresenceCount >= 2, 'HUD action proof must include two-tester bond gift presence');
assert(browserPresence.data?.hudAction?.state?.bondGiftRibbonClaimed === true, 'HUD action proof must include the no-real-value bond gift ribbon proof');
assert(browserPresence.data?.hudAction?.state?.nameBannerProof === true, 'HUD action proof must include the name banner proof');
assert(browserPresence.data?.hudAction?.state?.nameBannerRiteId === 'jade-name-banner-rite', 'HUD action proof must include the Jade Name Banner Rite id');
assert(browserPresence.data?.hudAction?.state?.nameBannerRiteName === 'Jade Name Banner Rite', 'HUD action proof must include the Jade Name Banner Rite name');
assert(browserPresence.data?.hudAction?.state?.nameBannerScore >= 46, 'HUD action proof must include a passing name banner score');
assert(browserPresence.data?.hudAction?.state?.nameBannerRequiredScore === 46, 'HUD action proof must include the name banner requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.nameBannerSpiritIds) && browserPresence.data.hudAction.state.nameBannerSpiritIds.length === 3, 'HUD action proof must include the full name banner roster');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.nameBannerTitles) && browserPresence.data.hudAction.state.nameBannerTitles.includes('Lirabao Lanternheart'), 'HUD action proof must include the Lirabao Lanternheart title');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.nameBannerTitles) && browserPresence.data.hudAction.state.nameBannerTitles.includes('Jintari Goldleaf Step'), 'HUD action proof must include the Jintari Goldleaf Step title');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.nameBannerTitles) && browserPresence.data.hudAction.state.nameBannerTitles.includes('Aozhen Skybell Veil'), 'HUD action proof must include the Aozhen Skybell Veil title');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.nameBannerJournalSpiritIds) && browserPresence.data.hudAction.state.nameBannerJournalSpiritIds.length === 3, 'HUD action proof must include 3/3 name banner journal proof');
assert(browserPresence.data?.hudAction?.state?.nameBannerPresenceCount >= 2, 'HUD action proof must include two-tester name banner presence');
assert(browserPresence.data?.hudAction?.state?.nameBannerTagClaimed === true, 'HUD action proof must include the no-real-value name banner tag proof');
...
assert(browserPresence.data?.hudAction?.state?.weatherVeilProof === true, 'HUD action proof must include the weather veil proof');
assert(browserPresence.data?.hudAction?.state?.weatherVeilId === 'jade-weather-veil', 'HUD action proof must include the Jade Weather Veil id');
assert(browserPresence.data?.hudAction?.state?.weatherVeilName === 'Jade Weather Veil', 'HUD action proof must include the Jade Weather Veil name');
assert(browserPresence.data?.hudAction?.state?.weatherVeilScore >= 36, 'HUD action proof must include a passing weather veil score');
assert(browserPresence.data?.hudAction?.state?.weatherVeilRequiredScore === 36, 'HUD action proof must include the weather veil requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.weatherVeilRouteIds) && browserPresence.data.hudAction.state.weatherVeilRouteIds.includes('moonbridge-bamboo-trail'), 'HUD action proof must include the Moonbridge weather veil route');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.weatherVeilRouteIds) && browserPresence.data.hudAction.state.weatherVeilRouteIds.includes('cloudbell-reed-bank'), 'HUD action proof must include the Cloudbell weather veil route');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.weatherVeilConditionIds) && browserPresence.data.hudAction.state.weatherVeilConditionIds.includes('moonlit-mist'), 'HUD action proof must include moonlit mist weather veil proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.weatherVeilConditionIds) && browserPresence.data.hudAction.state.weatherVeilConditionIds.includes('goldleaf-rain'), 'HUD action proof must include goldleaf rain weather veil proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.weatherVeilConditionIds) && browserPresence.data.hudAction.state.weatherVeilConditionIds.includes('skybell-crosswind'), 'HUD action proof must include skybell crosswind weather veil proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.weatherVeilWindows) && browserPresence.data.hudAction.state.weatherVeilWindows.length >= 2, 'HUD action proof must include weather veil route windows');
assert(browserPresence.data?.hudAction?.state?.weatherVeilChartClaimed === true, 'HUD action proof must include the no-real-value weather veil chart');
assert(browserPresence.data?.hudAction?.state?.encounterRotationProof === true, 'HUD action proof must include the encounter rotation proof');
assert(browserPresence.data?.hudAction?.state?.encounterRotationId === 'jade-encounter-rotation', 'HUD action proof must include the Jade Encounter Rotation id');
assert(browserPresence.data?.hudAction?.state?.encounterRotationName === 'Jade Encounter Rotation', 'HUD action proof must include the Jade Encounter Rotation name');
assert(browserPresence.data?.hudAction?.state?.encounterRotationScore >= 45, 'HUD action proof must include a passing encounter rotation score');
assert(browserPresence.data?.hudAction?.state?.encounterRotationRequiredScore === 45, 'HUD action proof must include the encounter rotation requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationRouteIds) && browserPresence.data.hudAction.state.encounterRotationRouteIds.includes('moonbridge-bamboo-trail'), 'HUD action proof must include the Moonbridge encounter rotation route');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationRouteIds) && browserPresence.data.hudAction.state.encounterRotationRouteIds.includes('cloudbell-reed-bank'), 'HUD action proof must include the Cloudbell encounter rotation route');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationSpiritIds) && browserPresence.data.hudAction.state.encounterRotationSpiritIds.includes('lirabao'), 'HUD action proof must include Lirabao encounter rotation proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationSpiritIds) && browserPresence.data.hudAction.state.encounterRotationSpiritIds.includes('jintari'), 'HUD action proof must include Jintari encounter rotation proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationSpiritIds) && browserPresence.data.hudAction.state.encounterRotationSpiritIds.includes('aozhen'), 'HUD action proof must include Aozhen encounter rotation proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationLureItemIds) && browserPresence.data.hudAction.state.encounterRotationLureItemIds.includes('lantern-harmony-tea'), 'HUD action proof must include Lantern Harmony Tea in the encounter rotation');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationLureItemIds) && browserPresence.data.hudAction.state.encounterRotationLureItemIds.includes('jade-thread-charm'), 'HUD action proof must include Jade Thread Charm in the encounter rotation');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.encounterRotationWindows) && browserPresence.data.hudAction.state.encounterRotationWindows.length >= 2, 'HUD action proof must include encounter rotation windows');
assert(browserPresence.data?.hudAction?.state?.encounterRotationScrollClaimed === true, 'HUD action proof must include the no-real-value encounter rotation scroll');
...
assert(browserPresence.data?.hudAction?.state?.routeCharterProof === true, 'HUD action proof must include the route charter proof');
assert(browserPresence.data?.hudAction?.state?.routeCharterId === 'jade-route-charter', 'HUD action proof must include the Jade Route Charter id');
assert(browserPresence.data?.hudAction?.state?.routeCharterName === 'Jade Route Charter', 'HUD action proof must include the Jade Route Charter name');
assert(browserPresence.data?.hudAction?.state?.routeCharterScore >= 40, 'HUD action proof must include a passing route charter score');
assert(browserPresence.data?.hudAction?.state?.routeCharterRequiredScore === 40, 'HUD action proof must include the route charter requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.routeCharterRouteIds) && browserPresence.data.hudAction.state.routeCharterRouteIds.includes('moonbridge-bamboo-trail'), 'HUD action proof must include the Moonbridge route charter route');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.routeCharterRouteIds) && browserPresence.data.hudAction.state.routeCharterRouteIds.includes('cloudbell-reed-bank'), 'HUD action proof must include the Cloudbell route charter route');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.routeCharterPartyIds) && browserPresence.data.hudAction.state.routeCharterPartyIds.includes('lirabao'), 'HUD action proof must include Lirabao route charter party proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.routeCharterPartyIds) && browserPresence.data.hudAction.state.routeCharterPartyIds.includes('jintari'), 'HUD action proof must include Jintari route charter party proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.routeCharterPartyIds) && browserPresence.data.hudAction.state.routeCharterPartyIds.includes('aozhen'), 'HUD action proof must include Aozhen route charter party proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.routeCharterProofIds) && browserPresence.data.hudAction.state.routeCharterProofIds.includes('jade-cloudbell-waystone'), 'HUD action proof must include waystone prerequisite proof for the route charter');
assert(browserPresence.data?.hudAction?.state?.routeCharterPresenceCount >= 2, 'HUD action proof must include two-tester route charter presence');
assert(browserPresence.data?.hudAction?.state?.routeCharterSlipClaimed === true, 'HUD action proof must include the no-real-value route charter slip proof');
...
assert(browserPresence.data?.hudAction?.state?.encounterAtlasScore >= 53, 'HUD action proof must include a passing encounter atlas score');
assert(browserPresence.data?.hudAction?.state?.encounterAtlasRequiredScore === 53, 'HUD action proof must include the encounter atlas requirement');
...
assert(browserPresence.data?.hudAction?.state?.encounterRotationId === 'jade-encounter-rotation', 'HUD action proof must preserve encounter rotation proof for the atlas');
assert(browserPresence.data?.hudAction?.state?.weatherVeilId === 'jade-weather-veil', 'HUD action proof must preserve weather veil proof for the atlas');
...
assert(browserPresence.data?.hudAction?.state?.habitatCensusProof === true, 'HUD action proof must include the Jade Habitat Census proof');
assert(browserPresence.data?.hudAction?.state?.habitatCensusId === 'jade-habitat-census', 'HUD action proof must include the Jade Habitat Census id');
assert(browserPresence.data?.hudAction?.state?.habitatCensusName === 'Jade Habitat Census', 'HUD action proof must include the Jade Habitat Census name');
assert(browserPresence.data?.hudAction?.state?.habitatCensusScore >= 49, 'HUD action proof must include a passing habitat census score');
assert(browserPresence.data?.hudAction?.state?.habitatCensusRequiredScore === 49, 'HUD action proof must include the habitat census requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusRouteIds) && browserPresence.data.hudAction.state.habitatCensusRouteIds.includes('moonbridge-bamboo-trail'), 'HUD action proof must include Moonbridge habitat census route proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusRouteIds) && browserPresence.data.hudAction.state.habitatCensusRouteIds.includes('cloudbell-reed-bank'), 'HUD action proof must include Cloudbell habitat census route proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusSpiritIds) && browserPresence.data.hudAction.state.habitatCensusSpiritIds.includes('lirabao'), 'HUD action proof must include Lirabao habitat census observation proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusSpiritIds) && browserPresence.data.hudAction.state.habitatCensusSpiritIds.includes('jintari'), 'HUD action proof must include Jintari habitat census observation proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusSpiritIds) && browserPresence.data.hudAction.state.habitatCensusSpiritIds.includes('aozhen'), 'HUD action proof must include Aozhen habitat census observation proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusCareLoggedSpiritIds) && browserPresence.data.hudAction.state.habitatCensusCareLoggedSpiritIds.includes('lirabao'), 'HUD action proof must include Lirabao habitat census care log');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusCareLoggedSpiritIds) && browserPresence.data.hudAction.state.habitatCensusCareLoggedSpiritIds.includes('jintari'), 'HUD action proof must include Jintari habitat census care log');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.habitatCensusCareLoggedSpiritIds) && browserPresence.data.hudAction.state.habitatCensusCareLoggedSpiritIds.includes('aozhen'), 'HUD action proof must include Aozhen habitat census care log');
assert(browserPresence.data?.hudAction?.state?.habitatCensusSealClaimed === true, 'HUD action proof must include the no-real-value habitat census seal proof');
...
assert(browserPresence.data?.hudAction?.state?.battleChronicleProof === true, 'HUD action proof must include the Jade Battle Chronicle proof');
assert(browserPresence.data?.hudAction?.state?.battleChronicleId === 'jade-battle-chronicle', 'HUD action proof must include the Jade Battle Chronicle id');
assert(browserPresence.data?.hudAction?.state?.battleChronicleName === 'Jade Battle Chronicle', 'HUD action proof must include the Jade Battle Chronicle name');
assert(browserPresence.data?.hudAction?.state?.battleChronicleScore >= 72, 'HUD action proof must include a passing battle chronicle score');
assert(browserPresence.data?.hudAction?.state?.battleChronicleRequiredScore === 72, 'HUD action proof must include the battle chronicle requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChroniclePartyIds) && browserPresence.data.hudAction.state.battleChroniclePartyIds.includes('lirabao'), 'HUD action proof must include Lirabao battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChroniclePartyIds) && browserPresence.data.hudAction.state.battleChroniclePartyIds.includes('jintari'), 'HUD action proof must include Jintari battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChroniclePartyIds) && browserPresence.data.hudAction.state.battleChroniclePartyIds.includes('aozhen'), 'HUD action proof must include Aozhen battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChronicleProofIds) && browserPresence.data.hudAction.state.battleChronicleProofIds.includes('jade-dojo-ladder'), 'HUD action proof must include dojo battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChronicleProofIds) && browserPresence.data.hudAction.state.battleChronicleProofIds.includes('jade-banner-tournament'), 'HUD action proof must include tournament battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChronicleProofIds) && browserPresence.data.hudAction.state.battleChronicleProofIds.includes('jade-rival-circle'), 'HUD action proof must include rival battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChronicleProofIds) && browserPresence.data.hudAction.state.battleChronicleProofIds.includes('jade-sifu-council'), 'HUD action proof must include sifu council battle chronicle proof');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.battleChronicleProofIds) && browserPresence.data.hudAction.state.battleChronicleProofIds.includes('jade-summit-circuit'), 'HUD action proof must include summit battle chronicle proof');
assert(browserPresence.data?.hudAction?.state?.battleChroniclePresenceCount >= 2, 'HUD action proof must include battle chronicle two-tester presence');
assert(browserPresence.data?.hudAction?.state?.battleChronicleSealClaimed === true, 'HUD action proof must include the no-real-value battle chronicle seal proof');
...
assert(browserPresence.data?.hudAction?.state?.questLedgerProof === true, 'HUD action proof must include the Jade Quest Ledger proof');
assert(browserPresence.data?.hudAction?.state?.questLedgerId === 'jade-quest-ledger', 'HUD action proof must include the Jade Quest Ledger id');
assert(browserPresence.data?.hudAction?.state?.questLedgerName === 'Jade Quest Ledger', 'HUD action proof must include the Jade Quest Ledger name');
assert(browserPresence.data?.hudAction?.state?.questLedgerScore >= 40, 'HUD action proof must include a passing quest ledger score');
assert(browserPresence.data?.hudAction?.state?.questLedgerRequiredScore === 40, 'HUD action proof must include the quest ledger requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.questLedgerAcceptedQuestIds) && browserPresence.data.hudAction.state.questLedgerAcceptedQuestIds.length === 3, 'HUD action proof must include all accepted quest ledger ids');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.questLedgerCompletedQuestIds) && browserPresence.data.hudAction.state.questLedgerCompletedQuestIds.length === 3, 'HUD action proof must include all completed quest ledger ids');
assert(browserPresence.data?.hudAction?.state?.questLedgerSealClaimed === true, 'HUD action proof must include the no-real-value quest ledger seal proof');
assert(browserPresence.data?.hudAction?.state?.dialogueScrollProof === true, 'HUD action proof must include the Jade Dialogue Scroll proof');
assert(browserPresence.data?.hudAction?.state?.dialogueScrollId === 'jade-dialogue-scroll', 'HUD action proof must include the Jade Dialogue Scroll id');
assert(browserPresence.data?.hudAction?.state?.dialogueScrollName === 'Jade Dialogue Scroll', 'HUD action proof must include the Jade Dialogue Scroll name');
assert(browserPresence.data?.hudAction?.state?.dialogueScrollScore >= 40, 'HUD action proof must include a passing dialogue scroll score');
assert(browserPresence.data?.hudAction?.state?.dialogueScrollRequiredScore === 40, 'HUD action proof must include the dialogue scroll requirement');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.dialogueScrollBeatIds) && browserPresence.data.hudAction.state.dialogueScrollBeatIds.length === 3, 'HUD action proof must include all original dialogue beat ids');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.dialogueScrollSpeakers) && browserPresence.data.hudAction.state.dialogueScrollSpeakers.includes('Sifu Narao'), 'HUD action proof must include Sifu Narao as a dialogue speaker');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.dialogueScrollSpeakers) && browserPresence.data.hudAction.state.dialogueScrollSpeakers.includes('Warden Meilin'), 'HUD action proof must include Warden Meilin as a dialogue speaker');
assert(Array.isArray(browserPresence.data?.hudAction?.state?.dialogueScrollSpeakers) && browserPresence.data.hudAction.state.dialogueScrollSpeakers.includes('Keeper Haoran'), 'HUD action proof must include Keeper Haoran as a dialogue speaker');
assert(browserPresence.data?.hudAction?.state?.dialogueScrollSealClaimed === true, 'HUD action proof must include the no-real-value dialogue scroll seal proof');
...
assert(browserPresence.data?.hudAction?.state?.wayfarerChronicleScore >= 77, 'HUD action proof must include a passing chronicle score');
assert(browserPresence.data?.hudAction?.state?.wayfarerChronicleRequiredScore === 77, 'HUD action proof must include the wayfarer chronicle requirement');
...
assert(browserPresence.data?.hudAction?.state?.guildAscensionScore >= 66, 'HUD action proof must include a passing guild ascension score');
assert(browserPresence.data?.hudAction?.state?.guildAscensionRequiredScore === 66, 'HUD action proof must include the guild ascension score requirement');
...
assert(browserPresence.data?.hudAction?.state?.marketReceiptId === 'jade-court-market-receipt', 'HUD action proof must include the Jade Court Market Receipt id');
assert(browserPresence.data?.hudAction?.state?.marketReceiptName === 'Jade Court Market Receipt', 'HUD action proof must include the Jade Court Market Receipt name');
assert(browserPresence.data?.hudAction?.state?.marketReceiptItemId === 'jade-thread-charm', 'HUD action proof must include the market receipt item');
assert(browserPresence.data?.hudAction?.state?.marketReceiptScore >= 16, 'HUD action proof must include a passing market receipt score');
assert(browserPresence.data?.hudAction?.state?.marketReceiptRequiredScore === 16, 'HUD action proof must include the market receipt requirement');
assert(browserPresence.data?.hudAction?.state?.marketReceiptClaimed === true, 'HUD action proof must include the no-real-value market receipt proof');
...
assert(browserPresence.data?.hudAction?.state?.canaryOperationReviewProof === true, 'HUD action proof must include Canary finality review');
assert(browserPresence.data?.hudAction?.state?.canaryOperationState === 'PENDING', 'HUD action proof must keep Canary finality review pending in preview-stub mode');
assert(browserPresence.data?.hudAction?.state?.canaryOperationFinalized === false, 'HUD action proof must not mark preview-stub Canary operations finalized');
assert(browserPresence.data?.hudAction?.state?.canaryInventoryCredited === false, 'HUD action proof must not credit inventory from Canary finality review');
...
assert(alphaContentSource.includes('MARKET_GUILD_RECEIPTS'), 'Jade Court Market Receipt table must be present');
assert(alphaContentSource.includes('Jade Court Market Receipt'), 'Jade Court Market Receipt content must be present');
assert(alphaContentSource.includes('jade-market-receipt'), 'Jade Market Receipt item must be present');
assert(alphaContentSource.includes('resolveMarketGuildReceipt'), 'market receipt resolver must be present');
assert(alphaContentSource.includes('market-guild-receipt'), 'market receipt source id must be present');
assert(alphaContentSource.includes('marketReceiptProof'), 'market receipt proof must feed provision and guild capstones');
assert(browserBridgeSource.includes('data-alpha-action="market.guild_receipt"'), 'HUD must expose the market receipt action button');
assert(browserBridgeSource.includes('resolveMarketGuildReceipt'), 'HUD bridge must call the market receipt resolver');
assert(browserBridgeSource.includes('marketReceiptClaimed'), 'HUD bridge must persist the no-real-value market receipt proof');
assert(alphaContentSource.includes('SPIRIT_PROVISION_CATALOGS'), 'Jade Provision Catalog table must be present');
assert(alphaContentSource.includes('Jade Provision Catalog'), 'Jade Provision Catalog content must be present');
assert(alphaContentSource.includes('jade-provision-catalog-seal'), 'Jade Provision Catalog Seal item must be present');
assert(alphaContentSource.includes('item-provision-catalog'), 'provision catalog source id must be present');
assert(alphaContentSource.includes('provisionCatalogProof'), 'provision catalog proof must feed guild capstones');
assert(browserBridgeSource.includes('data-alpha-action="item.provision_catalog"'), 'HUD must expose the provision catalog action button');
assert(browserBridgeSource.includes('resolveSpiritProvisionCatalog'), 'HUD bridge must call the provision catalog resolver');
assert(browserBridgeSource.includes('provisionCatalogSealClaimed'), 'HUD bridge must persist the no-real-value provision catalog seal proof');
assert(alphaContentSource.includes('SPIRIT_BATTLE_KITS'), 'Jade Battle Kit table must be present');
assert(alphaContentSource.includes('Jade Battle Kit'), 'Jade Battle Kit content must be present');
assert(alphaContentSource.includes('jade-battle-kit-tag'), 'Jade Battle Kit Tag item must be present');
assert(alphaContentSource.includes('item-battle-kit'), 'battle kit source id must be present');
assert(alphaContentSource.includes('battleKitProof'), 'battle kit proof must feed guild capstones');
assert(browserBridgeSource.includes('data-alpha-action="item.battle_kit"'), 'HUD must expose the battle kit action button');
assert(browserBridgeSource.includes('resolveSpiritBattleKit'), 'HUD bridge must call the battle kit resolver');
assert(browserBridgeSource.includes('battleKitTagClaimed'), 'HUD bridge must persist the no-real-value battle kit tag proof');
assert(alphaContentSource.includes('SPIRIT_REMEDY_POUCHES'), 'Jade Remedy Pouch table must be present');
assert(alphaContentSource.includes('Jade Remedy Pouch'), 'Jade Remedy Pouch content must be present');
assert(alphaContentSource.includes('jade-remedy-pouch-tag'), 'Jade Remedy Pouch Tag item must be present');
assert(alphaContentSource.includes('item-remedy-pouch'), 'remedy pouch source id must be present');
assert(alphaContentSource.includes('remedyPouchProof'), 'remedy pouch proof must feed guild capstones');
assert(browserBridgeSource.includes('data-alpha-action="item.remedy_pouch"'), 'HUD must expose the remedy pouch action button');
assert(browserBridgeSource.includes('resolveSpiritRemedyPouch'), 'HUD bridge must call the remedy pouch resolver');
assert(browserBridgeSource.includes('remedyPouchTagClaimed'), 'HUD bridge must persist the no-real-value remedy pouch tag proof');
assert(alphaContentSource.includes('MOCHI_QUEST_LEDGERS'), 'Jade Quest Ledger table must be present');
assert(alphaContentSource.includes('Jade Quest Ledger'), 'Jade Quest Ledger content must be present');
assert(alphaContentSource.includes('jade-quest-ledger-seal'), 'Jade Quest Ledger Seal item must be present');
assert(alphaContentSource.includes('resolveMochiQuestLedger'), 'quest ledger resolver must be present');
assert(alphaContentSource.includes('quest-ledger'), 'quest ledger source id must be present');
assert(alphaContentSource.includes('questLedgerProof'), 'quest ledger proof must feed story and guild capstones');
assert(browserBridgeSource.includes('data-alpha-action="quest.ledger_record"'), 'HUD must expose the quest ledger action button');
assert(browserBridgeSource.includes('data-quest-ledger-label'), 'HUD must expose the quest ledger status label');
assert(browserBridgeSource.includes('resolveMochiQuestLedger'), 'HUD bridge must call the quest ledger resolver');
assert(browserBridgeSource.includes('questLedgerSealClaimed'), 'HUD bridge must persist the no-real-value quest ledger seal proof');
...
assert(alphaContentSource.includes('SPIRIT_ROUTE_CHARTERS'), 'Jade Route Charter table must be present');
assert(alphaContentSource.includes('Jade Route Charter'), 'Jade Route Charter content must be present');
assert(alphaContentSource.includes('jade-route-charter-slip'), 'Jade Route Charter Slip item must be present');
assert(alphaContentSource.includes('resolveSpiritRouteCharter'), 'route charter resolver must be present');
assert(alphaContentSource.includes('world-route-charter'), 'route charter source id must be present');
assert(alphaContentSource.includes('routeCharterProof'), 'route charter proof must feed later guild capstones');
...
assert(alphaContentSource.includes('Jade Encounter Rotation'), 'Jade Encounter Rotation content must be present');
assert(alphaContentSource.includes('jade-encounter-rotation'), 'Jade Encounter Rotation id/item must be present');
assert(alphaContentSource.includes('resolveSpiritEncounterRotation'), 'encounter rotation resolver must be present');
assert(alphaContentSource.includes('spirit-encounter-rotation'), 'spirit encounter rotation source id must be present');
assert(alphaContentSource.includes('encounterRotationProof'), 'encounter rotation proof must feed later capstones');
...
assert(alphaContentSource.includes('Jade Habitat Census'), 'Jade Habitat Census content must be present');
assert(alphaContentSource.includes('jade-habitat-census-seal'), 'Jade Habitat Census Seal item must be present');
assert(alphaContentSource.includes('resolveSpiritHabitatCensus'), 'habitat census resolver must be present');
assert(alphaContentSource.includes('spirit-habitat-census'), 'spirit habitat census source id must be present');
assert(alphaContentSource.includes('habitatCensusProof'), 'habitat census proof must feed later capstones');
...
assert(alphaContentSource.includes('jade-battle-chronicle-seal'), 'Jade Battle Chronicle Seal item must be present');
assert(alphaContentSource.includes('resolveSpiritBattleChronicle'), 'battle chronicle resolver must be present');
assert(alphaContentSource.includes('battle-chronicle'), 'battle chronicle source id must be present');
assert(alphaContentSource.includes('battleChronicleProof'), 'battle chronicle proof must feed HUD state');
...
assert(browserBridgeSource.includes('data-alpha-action="world.route_charter"'), 'HUD must expose the route charter action button');
assert(browserBridgeSource.includes('data-route-charter-label'), 'HUD must expose the route charter label');
assert(browserBridgeSource.includes('resolveSpiritRouteCharter'), 'HUD bridge must call the route charter resolver');
assert(browserBridgeSource.includes('routeCharterSlipClaimed'), 'HUD bridge must persist the no-real-value route charter slip proof');
...
assert(browserBridgeSource.includes('data-alpha-action="battle.battle_chronicle"'), 'HUD must expose the battle chronicle action button');
assert(browserBridgeSource.includes('data-battle-chronicle-label'), 'HUD must expose the battle chronicle label');
assert(browserBridgeSource.includes('resolveSpiritBattleChronicle'), 'HUD bridge must call the battle chronicle resolver');
...
assert(alphaContentSource.includes('SPIRIT_ROSTER_CABINETS'), 'Jade Roster Cabinet content table must be present');
assert(alphaContentSource.includes('Jade Roster Cabinet'), 'Jade Roster Cabinet content must be present');
assert(alphaContentSource.includes('jade-roster-cabinet-tag'), 'Jade Roster Cabinet Tag item must be present');
assert(alphaContentSource.includes('resolveSpiritRosterCabinet'), 'roster cabinet resolver must be present');
assert(alphaContentSource.includes('spirit-roster-cabinet'), 'spirit roster cabinet source id must be present');
assert(alphaContentSource.includes('rosterCabinetProof'), 'roster cabinet proof must feed later capstones');
assert(browserBridgeSource.includes('data-alpha-action="spirit.roster_cabinet"'), 'HUD must expose the roster cabinet action button');
assert(browserBridgeSource.includes('data-roster-cabinet-label'), 'HUD must expose the roster cabinet label');
assert(browserBridgeSource.includes('resolveSpiritRosterCabinet'), 'HUD bridge must call the roster cabinet resolver');
assert(browserBridgeSource.includes('rosterCabinetTagClaimed'), 'HUD state must record the no-real-value roster cabinet tag proof');
assert(alphaContentSource.includes('SPIRIT_BLOSSOM_CRADLES'), 'Jade Blossom Cradle content table must be present');
assert(alphaContentSource.includes('Jade Blossom Cradle'), 'Jade Blossom Cradle content must be present');
assert(alphaContentSource.includes('jade-blossom-cradle-ribbon'), 'Jade Blossom Cradle Ribbon item must be present');
assert(alphaContentSource.includes('resolveSpiritBlossomCradle'), 'blossom cradle resolver must be present');
assert(alphaContentSource.includes('spirit-blossom-cradle'), 'spirit blossom cradle source id must be present');
assert(alphaContentSource.includes('blossomCradleProof'), 'blossom cradle proof must feed later capstones');
assert(browserBridgeSource.includes('data-alpha-action="spirit.blossom_cradle"'), 'HUD must expose the blossom cradle action button');
assert(browserBridgeSource.includes('data-blossom-cradle-label'), 'HUD must expose the blossom cradle label');
assert(browserBridgeSource.includes('resolveSpiritBlossomCradle'), 'HUD bridge must call the blossom cradle resolver');
assert(browserBridgeSource.includes('blossomCradleRibbonClaimed'), 'HUD state must record the no-real-value blossom cradle ribbon proof');
...
assert(alphaContentSource.includes('Jade Weather Veil'), 'Jade Weather Veil content must be present');
assert(alphaContentSource.includes('jade-weather-veil-chart'), 'Jade Weather Veil Chart item must be present');
assert(alphaContentSource.includes('spirit-weather-veil'), 'weather veil source id must be present');
assert(alphaContentSource.includes('weatherVeilProof'), 'weather veil proof must feed encounter planning');
assert(browserBridgeSource.includes('data-alpha-action="world.weather_veil"'), 'HUD must expose the weather veil action button');
assert(browserBridgeSource.includes('data-weather-veil-label'), 'HUD must expose the weather veil label');
assert(browserBridgeSource.includes('resolveSpiritWeatherVeil'), 'HUD bridge must call the weather veil resolver');
assert(
  Array.from(
...
const visualChecklist = {
  ...
  canaryNoRealValueClarity: {
    status: browserPresence.data?.hudAction?.state?.canaryRequested === true
      && browserPresence.data?.hudAction?.state?.canaryReturnRequested === true
      && browserPresence.data?.hudAction?.state?.canaryOperationReviewProof === true
      && browserPresence.data?.hudAction?.state?.canaryInventoryCredited === false
      ? 'machine-supported'
      : 'blocked',
    records: 'Canary request, return preview, and finality review remain staged/no-real-value HUD flows with no inventory credit; funded-chain gates stay out of this visual pass.'
  },
  twoTabPresence: {
...
const report = {
  ...
  machineReview: {
    ...
    hudActionLoop: {
      ...
      rosterCabinet: browserPresence.data?.hudAction?.state?.rosterCabinetProof === true && browserPresence.data?.hudAction?.state?.rosterCabinetScore >= 30,
      blossomCradle: browserPresence.data?.hudAction?.state?.blossomCradleProof === true && browserPresence.data?.hudAction?.state?.blossomCradleScore >= 48,
      provisionSatchel: browserPresence.data?.hudAction?.state?.provisionProof === true,
      provisionCatalog: browserPresence.data?.hudAction?.state?.provisionCatalogProof === true && browserPresence.data?.hudAction?.state?.provisionCatalogScore >= 50,
      battleKit: browserPresence.data?.hudAction?.state?.battleKitProof === true && browserPresence.data?.hudAction?.state?.battleKitScore >= 48,
      remedyPouch: browserPresence.data?.hudAction?.state?.remedyPouchProof === true && browserPresence.data?.hudAction?.state?.remedyPouchScore >= 50,
      careCycle: browserPresence.data?.hudAction?.state?.careCycleProof === true,
      bondGift: browserPresence.data?.hudAction?.state?.bondGiftProof === true && browserPresence.data?.hudAction?.state?.bondGiftScore >= 36,
      nameBanner: browserPresence.data?.hudAction?.state?.nameBannerProof === true && browserPresence.data?.hudAction?.state?.nameBannerScore >= 46,
      ...
      weatherVeil: browserPresence.data?.hudAction?.state?.weatherVeilProof === true && browserPresence.data?.hudAction?.state?.weatherVeilScore >= 36,
      ...
      routeCharter: browserPresence.data?.hudAction?.state?.routeCharterProof === true && browserPresence.data?.hudAction?.state?.routeCharterScore >= 40,
      ...
      encounterRotation: browserPresence.data?.hudAction?.state?.encounterRotationProof === true && browserPresence.data?.hudAction?.state?.encounterRotationScore >= 45,
      encounterAtlas: browserPresence.data?.hudAction?.state?.encounterAtlasProof === true && browserPresence.data?.hudAction?.state?.encounterAtlasScore >= 53,
      habitatCensus: browserPresence.data?.hudAction?.state?.habitatCensusProof === true && browserPresence.data?.hudAction?.state?.habitatCensusScore >= 49,
      ...
      battleChronicle: browserPresence.data?.hudAction?.state?.battleChronicleProof === true && browserPresence.data?.hudAction?.state?.battleChronicleScore >= 72,
      ...
      questLedger: browserPresence.data?.hudAction?.state?.questLedgerProof === true && browserPresence.data?.hudAction?.state?.questLedgerScore >= 40,
      dialogueScroll: browserPresence.data?.hudAction?.state?.dialogueScrollProof === true && browserPresence.data?.hudAction?.state?.dialogueScrollScore >= 40,
      ...
      wayfarerChronicle: browserPresence.data?.hudAction?.state?.wayfarerChronicleProof === true && browserPresence.data?.hudAction?.state?.wayfarerChronicleScore >= 77,
      guildAscensionTrial: browserPresence.data?.hudAction?.state?.guildAscensionProof === true && browserPresence.data?.hudAction?.state?.guildAscensionScore >= 66,
      ...
      marketReceipt: browserPresence.data?.hudAction?.state?.marketReceiptProof === true && browserPresence.data?.hudAction?.state?.marketReceiptScore >= 16,
      ...
      canaryFinalityReview: browserPresence.data?.hudAction?.state?.canaryOperationReviewProof === true && browserPresence.data?.hudAction?.state?.canaryInventoryCredited === false
    }
  },
  visualChecklist,
  manualPromptGate: {
