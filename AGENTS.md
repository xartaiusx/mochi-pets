...
- Keep canonical runtime sprite IDs as `wayfarer`, `sifu-narao`, `spirit-lirabao`, `spirit-jintari`, `spirit-aozhen`, `chest`, `habitat-grove`, `journal-pavilion`, `party-banner`, `training-ring`, `quest-board`, `market-board`, `trade-post`, and `canary-shrine`.
...
- The town has collision, one NPC dialog, one chest/item pickup, spirit invitation/capture, journal records, party formation, no-injury spar ladder practice, spirit training, quest-board progress, a minimal HUD, and save-backed player state.
