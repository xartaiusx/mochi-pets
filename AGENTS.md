...
- The local alpha acceptance and load-smoke commands verify public endpoints and no-real-value fallback ledger writes. `npm run alpha:browser-presence` verifies two same-browser tabs render the game canvas, HUD, `Nearby: 2 testers` presence chip, and the HUD care/chat/emote/market/trade/Canary quick-action loop. It is local-only unless `MOCHI_SOCIAL_BROWSER_ALLOW_HOSTED_SMOKE=true` is set after explicit hosted-preview approval. `npm run alpha:enjin-operator-smoke` verifies the private Enjin operator route fails closed and does not submit live Enjin operations by default. `npm run alpha:built-server-smoke` starts the built Express runtime locally with throwaway env and catches server-bundle runtime issues before any deploy. `npm run alpha:external-gates` audits GitHub, Fly, Supabase, live game/site contract, and operator-confirmed Enjin readiness without printing secret values. Manual RPGJS sprite/movement inspection still completes the visual multiplayer gate.
