...
- Parent to Unity: `MOCHI_PETS_AUTH`, `MOCHI_PETS_SIGN_OUT`
- Unity to parent: `MOCHI_PETS_READY`, `MOCHI_PETS_AUTH_STATE`, `MOCHI_PETS_ERROR`

Unity receives only a Supabase access token from the parent. It calls the server-side `mochi-pets-unity-auth` Edge Function, then processes the returned Unity player tokens with Unity Authentication. Supabase service-role keys, Unity service credentials, game-server tokens, Enjin secrets, and provider admin credentials must never enter Unity code, browser code, logs, docs, or PR text.
...
The Supabase audit mirror is optional until provider setup is approved. If enabled, store `MOCHI_PETS_ALPHA_ACTION_URL` and `MOCHI_PETS_GAME_SERVER_TOKEN` in Unity Secret Manager for Cloud Code access only. Do not pass the game-server token from Unity WebGL, browser code, docs, logs, or PR comments.
