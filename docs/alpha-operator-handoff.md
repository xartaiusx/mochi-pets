...
- `npm run alpha:responsive-gameplay` passes for `/play`, `/embed`, and parent-iframe input ownership across the alpha viewport matrix, proving the HUD does not incoherently overlap gameplay and movement/action keys do not scroll the browser or parent page.
- When the Mochirii parent page is available, rerun responsive gameplay with `$env:MOCHI_SOCIAL_RESPONSIVE_SITE_BASE_URL="<local-or-preview-site-origin>"`, `$env:MOCHI_SOCIAL_RESPONSIVE_REQUIRE_SITE_IFRAME="true"`, and a private `$env:MOCHI_SOCIAL_TESTER_PASSWORD` only if the tester gate is active, then confirm `reports/alpha-responsive-gameplay.json` contains nine `siteIframeResults` for `/games/mochi-social`.
