...
- Before Alpha Preview Ready, rerun responsive gameplay with `$env:MOCHI_SOCIAL_RESPONSIVE_SITE_BASE_URL="<local-or-preview-site-origin>"`, `$env:MOCHI_SOCIAL_RESPONSIVE_REQUIRE_SITE_IFRAME="true"`, and a private `$env:MOCHI_SOCIAL_TESTER_PASSWORD` only if the tester gate is active, then confirm `reports/alpha-responsive-gameplay.json` contains nine `siteIframeResults` for the unlocked `/games/mochi-social` iframe. `npm run alpha:preview-ready` treats a skipped site iframe leg as a tester-entry blocker.
