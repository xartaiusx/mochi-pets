...
- Try the town loop: move, meet one Mochi Spirit, scout the Moonbridge route, practice a technique, study a tactic scroll, record the Jade Court rank proof, open the Moonwell Bloom growth proof, try the Jade Mirror affinity trial, care for it, view the profile status, add the local guild buddy proof, set the social status, inspect the HUD, send one local chat message, use one emote, create one test market listing, create one direct trade proof, and request the Canary certificate proof.
