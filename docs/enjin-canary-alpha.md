...
By default on this workstation the script checks the canonical `Mochi Creds` boundary first, then the legacy Desktop and Downloads locations. Set `MOCHI_SOCIAL_WALLET_DAEMON_PATH` to check a different binary. The script records only path, file size, SHA256, and `wallet-daemon --help` command names in `reports/wallet-daemon-local.json` and `reports/wallet-daemon-local.md`.
