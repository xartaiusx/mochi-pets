...
- Keep the Canary certificate, Jade Vault return, and Canary finality review UI visible with clear `configured-preview-stub` messaging.
- Record chain requests, return previews, and finality reviews as audit-only preview rows.
...
- Bond gift proof is no-real-value: the Jade Bond Gift Rite records `item.bond_gift` after the Jade Court Market Receipt, Jade Court Provision Satchel, Jade Court Care Cycle, full first-court roster, Jade Mooncake Box, Lantern Harmony Tea, Jade Thread Charm, profile/guild/status/chat readiness, and two-tester witness proof. The Jade Bond Gift Ribbon must not imply settlement, inventory credit, cashout, production value, or Enjin finality.
- Name banner proof is no-real-value: the Jade Name Banner Rite records `spirit.name_banner` after 3/3 journal records, Jade Court Spirit Compendium, Jade Court Roster Archive, Jade Roster Cabinet, Jade Bond Gift Rite, full first-court roster, and the original Mochirii titles Lirabao Lanternheart, Jintari Goldleaf Step, and Aozhen Skybell Veil. The Jade Name Banner Tag must not imply settlement, inventory credit, cashout, production value, or Enjin finality.
- Quest ledger and dialogue scroll proof are no-real-value: the Jade Quest Ledger records `quest.ledger_record` after all three first-court quest postings, journal records, route mastery, route patrol, market receipt, provision satchel, commission ledger, two-tester rally, and social readiness proofs. The Jade Dialogue Scroll records `story.dialogue_scroll` after the quest ledger and name banner, preserving three original Mochirii speaker beats before the story chapter. The Jade Quest Ledger Seal and Jade Dialogue Scroll Seal must not imply settlement, inventory credit, cashout, production value, or Enjin finality.
...
- Route charter proof is no-real-value: the Jade Route Charter Slip is audit-only and must not imply settlement, inventory credit, cashout, or production value.
...
- Battle chronicle proof is no-real-value: the Jade Battle Chronicle Seal is a local alpha proof only and records the no-injury battle-history chain after the dojo ladder, tournament, rival circle, Sifu council, summit circuit, technique codex, condition weave, affinity matrix, remedy pouch, two-tester witness, and battle transcript without inventory credit, payout, settlement, or Enjin finality.
...
- Blossom cradle proof is no-real-value: the Jade Blossom Cradle Ribbon records roster-wide care, bond, kinship, nursery, bloom, lineage, nurture, growth, and raising continuity before Chronicle and Ascension and must not imply settlement, inventory credit, or production value.
- Chain request stub works: visible Canary request, Jade Vault return preview, and Canary finality review record audit-only requests, explain `configured-preview-stub`, and show no inventory credit before `FINALIZED`.
