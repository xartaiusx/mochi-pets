...
`npm run alpha:browser-presence` opens two pages in one browser context and verifies both tabs render the game canvas, the alpha HUD, and a `Nearby: 2 testers` presence chip. It captures canvas screenshot signatures, sends movement keys in both tabs, verifies each canvas changes, and verifies the observer tab changes after first-tab movement. It also clicks the HUD invite, attune, journal, scout, route invite, circuit, dojo, tactic, rank, bloom, affinity trial, party, harmony, concord, care, train, spar, raise, quest, profile view, local guild buddy proof, social status, spirit inspect, chat, emote, fixed-list, direct-trade, and Canary certificate actions, then verifies the visible HUD and `mochiSocial.alphaState` update. The HUD proof must include Lirabao, Jintari, and Aozhen in roster/party, 3/3 journal records, Moonbridge and Cloudbell route proof, Jade Cloudbell route mastery proof, Triune Jade Harmony proof, Jade Echo Concord proof, and the completed first Mochirii quest chain. It writes `reports/alpha-browser-presence.json`.
...
`npm run alpha:visual-review` reads `reports/alpha-visual-snapshot.json`, `reports/alpha-browser-presence.json`, the PNGs, and the first-town map-object sources, then writes ignored no-secret `reports/alpha-visual-review.json` and `reports/alpha-visual-review.md`. It verifies screenshot dimensions and hashes, HUD/presence evidence, observer movement, HUD spirit invitation/journal/field-expedition/route-invitation/technique/tactic/rank/growth-rite/affinity-trial/party/harmony/concord/spar/training/quest/market/trade/Canary actions, required map-object IDs, and Jade Lantern Court habitat coverage. It keeps rendered NPC/guild-seal-chest/habitat prompt interaction as `pending-human-review`; it is a durable local review bundle, not a fake replacement for the manual prompt check.
