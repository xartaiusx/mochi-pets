...
The browser HUD loop also clicks `world.encounter_rotation` after route ecology and verifies the Rotation label, `encounterRotationProof`, route IDs, spirit IDs, lure IDs, rotation windows, two-tester witness, and Jade Encounter Rotation Scroll no-real-value state.
