...
`npm run alpha:preview-ready` writes `reports/alpha-preview-ready.json`, `reports/alpha-preview-ready.md`, and `C:\Users\xtyty\Desktop\Creds\mochi-social-alpha-preview-ready.md`. It proves the tester-entry lane separately from funded-chain Alpha RC gates by requiring current local evidence, a responsive gameplay report that checked the unlocked Mochirii `/games/mochi-social` iframe across all nine viewports with `MOCHI_SOCIAL_RESPONSIVE_REQUIRE_SITE_IFRAME=true`, current no-secret hygiene, completed manual prompt review, current handoff packets, synced game/site branches, and green `preview-live-gates` from an explicitly approved hosted external-gates run.
...
`npm run alpha:responsive-gameplay` opens `/play`, `/embed`, a synthetic parent page embedding `/embed`, and, when configured, the real Mochirii `/games/mochi-social` iframe across the alpha viewport matrix: 1920x1080, 1440x900, 1366x768, 1280x720, 1024x768, 932x430, 844x390, 430x932, and 390x844. It verifies the canvas and HUD render, captures ignored screenshots under `reports/responsive-gameplay/`, and writes `reports/alpha-responsive-gameplay.json`.

The gate checks for no horizontal overflow, no incoherent HUD panel overlap, central gameplay safe-area preservation, reachable first and last action buttons, focus visibility, canvas `touch-action`, overscroll containment, and editable chat behavior while movement keys are typed. It also focuses the game canvas and sends Arrow keys, WASD, Space, and Enter to prove gameplay controls prevent default browser scrolling only while the gameplay surface is focused, do not scroll the browser page, HUD scroll regions, parent page, or iframe page, and record per-key keydown evidence in `reports/alpha-responsive-gameplay.json`. The same audit verifies editable inputs preserve movement/action characters without prevented keydown events, Tab remains usable for focus movement, and unhandled keys are not hijacked. It is local-only by default; set `MOCHI_SOCIAL_RESPONSIVE_ALLOW_HOSTED_SMOKE=true` only after explicit hosted-preview approval.
