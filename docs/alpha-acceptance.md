...
The browser HUD loop also clicks `spirit.roster_cabinet` after the Jade Lineage Register and verifies the Jade Roster Cabinet label, `rosterCabinetProof`, cabinet score, full roster, three party IDs, three storage labels, and Jade Roster Cabinet Tag no-real-value state.
