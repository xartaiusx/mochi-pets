...
- Content-only proof loops such as the Jade Court Craft Writ, Jade Cloudbell Waystone, Jade Moonwell Nurture Rite, Jade Kinship Album, Jade Capture Rite, Jade Banner Tournament, Jade Scroll Story Chapter, and Jade Insignia Case may add HUD labels/actions and tests without an asset-ledger row when no runtime art file or source card is created.
